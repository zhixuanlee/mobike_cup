...
def draw(data, n_sep=300, mult=1, shape=None):
    ...
    if shape is not None:
        ...
        temp = data[(data['start_loc_lon'] > (lon_ave - shape * lon_std)) & (
        data['start_loc_lon'] < (lon_ave + shape * lon_std))]
        temp = temp[(temp['start_loc_lat'] > (lat_ave - shape * lat_std)) & (
        temp['start_loc_lat'] < (lat_ave + shape * lat_std))]
...
def train_tztq(train):
    ...
    # 换算为经纬度
    zh.loc[:, 'start_jw'] = zh.max_dd.map(pygeohash.decode_exactly)
    zh.loc[:, 'start_j'] = zh.start_jw.map(lambda x: x[0])
    zh.loc[:, 'start_w'] = zh.start_jw.map(lambda x: x[1])
    zh.loc[:, 'end_jw'] = zh.min_dd.map(pygeohash.decode_exactly)
...
def jsjl(zh):
    ...
    # 换算为经纬度
    js.loc[:, 'start_jw'] = js.iloc[:, 0].map(pygeohash.decode_exactly)
    js.loc[:, 'start_j'] = js.start_jw.map(lambda x: x[0])
    js.loc[:, 'start_w'] = js.start_jw.map(lambda x: x[1])
    js.loc[:, 'end_jw'] = js.iloc[:, 1].map(pygeohash.decode_exactly)
...
example = pd.read_csv('F:\data science\data\mobike/sample_submission.csv',header=None)
